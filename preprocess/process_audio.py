...
#             ]
#         )
#     return torchaudio.sox_effects.apply_effects_file(path = filepath,effects=effects)
# .apply_effects_file(filepath, effects=effects)

import librosa
import torch
from typing import Tuple
# Program to get wave form 
def get_waveform(filepath: str, resample_rate: int = None) -> Tuple[torch.Tensor, int]:
    """
    Loads a waveform from a filepath and resamples it to a specified rate.
    :param filepath: Path to the audio file.
    ...
    # Apply resampling if resample_rate is provided
    if resample_rate:
...
# Example usage:
# filepath = r"C:\Users\gandl\Documents\AI generaated\real\LJ002-0046.wav"
# resample_rate = 16000  # Change as needed
# waveform, sample_rate = get_waveform(filepath, resample_rate)
...
def test():
    audio_file = Path(
        r"C:\Users\gandl\Documents\AI generaated\real\LJ002-0046.wav"
    )
